...
// Agent-triggerable variant: the SAME report, but consolidated across every ticket/agreement
// visible to the current user (RBAC-scoped), instead of a single document. Global — rendered
// once at the app root and opened via store.openCommentsReportOpen, so the agent chat can
// trigger it from anywhere without needing to be inside a specific ticket workspace.
export function ConsolidatedOpenCommentsModal({ onClose }: { onClose: () => void }) {
  const messages = useStore((s) => s.messages)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-6" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-pop">
        <div className="mb-1 flex items-center gap-2">
          <AtSign size={15} className="text-amber-500" />
          <h3 className="text-[14px] font-bold text-slate-800">Consolidated open comments</h3>
          <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700">{rows.length}</span>
          <div className="ml-auto flex items-center gap-1.5">
            <Button size="sm" variant="outline" icon={<Download size={12} />} onClick={exportReport}>Export report</Button>
            <button onClick={onClose} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100"><X size={15} /></button>
        ...
        <p className="mb-3 text-[11.5px] text-slate-400">Every unresolved comment across every matter visible to you — with who's tagged and how long it has waited.</p>
        <div className="space-y-1.5">
          {rows.length === 0 && <div className="py-6 text-center text-[12.5px] text-slate-400">No open comments anywhere. 🎉</div>}
          {rows.map(({ m, days, label, ticket, agreement }) => (
            <div key={m.id} className="rounded-lg border border-slate-100 px-3 py-2 hover:bg-slate-50">
              <button
                onClick={() => { onClose(); if (agreement) openAgreement(agreement.id, 'review'); else if (ticket) openTicket(ticket.id) }}
                className="flex w-full items-start gap-2.5 text-left"
              >
                <Avatar userId={m.author_id} size={20} />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[11.5px] font-semibold text-slate-500">{label}</div>
                  <div className="truncate text-[12.5px] text-slate-700">{m.body}</div>
                  <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-400">
                    {m.provision_reference && <Chip className="bg-slate-100 text-slate-500 ring-slate-300/30">{m.provision_reference}</Chip>}
                    {m.mentions?.length ? <span className="font-semibold text-amber-600">@ {m.mentions.map((id) => userById(id)?.name.split(' ')[0]).join(', ')}</span> : <span>no one tagged</span>}
