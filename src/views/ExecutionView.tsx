...
export function SingleAgreementExecution({ agreementId, compact }: { agreementId: string; compact?: boolean }) {
  ...
  const signStep: 'done' | 'active' | 'todo' = executed ? 'done' : approved ? 'active' : 'todo'
      ...
      <Step n={1} title="Attorney / approval chain" state={approvalStep}>
        {!approval ? (
          <div className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2.5">
            ...
            <Button size="sm" variant="outline" icon={<ShieldCheck size={13} />} onClick={() => createApproval(agreementId, 'external_delivery')}>Request approvals</Button>
          </div>
        ) : (
      ...
      <Step n={2} title="Electronic signature (DocuSign)" state={signStep}>
        {!approved ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-[11.5px] text-slate-400"><span className="font-mono">{envelope.id}</span> · created {fmtDateTime(envelope.created_date)}{envelope.mode ? ` · ${envelope.mode === 'combined' ? 'batch' : 'individual'}` : ''}</div>
      ...
      <Step n={3} title="Archive & generate deal summary" state={archiveStep}>
        {executed ? (
          <div className="space-y-2">
            ...
            <Button size="sm" variant="outline" icon={<ScrollText size={13} />} onClick={() => openCanvas({ view: 'deal_summary', dealSummaryId: agreementId })}>View the generated deal summary</Button>
          </div>
        ) : (
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      <div className="mx-auto max-w-2xl">
        ...
        <SingleAgreementExecution agreementId={id} />
      </div>
    </div>
  )
}
