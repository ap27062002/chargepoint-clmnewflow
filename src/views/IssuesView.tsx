...
import { Sparkles, FileText, ChevronRight, Lock, BookOpen } from 'lucide-react'
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <div className="flex items-center justify-between border-t border-slate-100 px-4 py-2.5">
        <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-400"><Lock size={12} /> Read-only — decide dispositions in the Review tab</div>
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      <div className="space-y-3">
        {visible.map((d) => <DeviationCard key={d.id} d={d} onViewInDoc={() => onViewInDoc(d.id)} onOpenPlaybook={openPlaybook} />)}
        {hasMore && (
          <div className="flex items-center justify-center gap-3 pt-1">
            <span className="text-[11.5px] text-slate-400">Showing {visible.length} of {total} issues — windowed for performance</span>
