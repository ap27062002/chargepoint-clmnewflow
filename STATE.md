...
**Live:** https://chargepoint-clm.vercel.app · **Current build:** `build-8` (git tag) · repo clean.
...
## Done (builds 1→7)
1 original 14-screen app · 2 agent-first redesign (hero + side-by-side, ⌘K palette) · 3 RBAC (starters/palette/agent-denial/row-scoping) · 4 "make it real" (live-Claude wiring, real track-changes editor, e-sign/exec, approvals/routing/SLA engines, computed analytics, version diff) · 5 visible lifecycle stage tracker + inline approval chain · 6 redesigned Deal Summary + signed/executed document (view + download + DocuSign completion certificate) · 7 execution auto-resolves open deviations · 8 UX refinements: left nav (Agent/Dashboard/Playbook, RBAC-gated), click-to-open agent (no auto-open — user clicks the artifact chip), removed Negotiation stage, leaner StageTracker, Agreements pill only for multi-agreement tickets, right rail defaults to AI Assistant, starter renames.
